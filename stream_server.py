...
async def scrub_meta(v: int | None = None):
    """Layout for the hover thumbnails."""
    from fastapi import Response
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            def advance_one():
                """Burn one effective frame without decoding. Returns False at EOF."""
                ...
                while True:
                    while not cmd_queue.empty():
                        ...
                        if msg.get("type") == "pause":
                            ...
                            # Client's current decoded-frame backlog
                            try:
...
if __name__ == "__main__":
    ...
    playback.add_argument(
        "--no-thumbnails",
        action="store_true", default=False,
        help="Turn off the hover thumbnails on the seek bar."
    )
